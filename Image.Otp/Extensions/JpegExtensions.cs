...
public static class JpegExtensions
{
    private sealed class Accumulator
    {
        ...
        public Dictionary<(byte Class, byte Id), HuffmanTable> HuffmanTables { get; init; } = [];
                ...
                if (JpegMarkers.HasLengthData(marker))
                {
                    ...
                    switch (marker)
                    {
                        ...
                        case JpegMarkers.DHT:
                            ProcessDHT(stream, stream.Position + length, accumulator);
                            break;
    ...
    private static void ProcessDHT(Stream stream, long endPosition, Accumulator acc)
    {
        if (stream.Position > endPosition)
            throw new InvalidDataException("Stream position exceeds segment boundary.");
        ...
        Span<uint> workspace = stackalloc uint[256];
        while (stream.Position < endPosition)
        {
            ...
            acc.HuffmanTables[(tc, th)] = new HuffmanTable([0, .. lengths], symbols, workspace);
        }
    }
        ...
        Span<T> output) where T : unmanaged, IPixel<T>
    {
        ...
        var huffPool = ArrayPool<HuffmanTable>.Shared;
        var dcTables = huffPool.Rent(4);
        var acTables = huffPool.Rent(4);
